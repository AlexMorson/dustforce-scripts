...
class clone {
        ...
        if (
            ...
            if (last_state.air_charges == new_state.air_charges) {
                if (last_state.ground) {
                    float direction = sgn(new_state.x_speed);
                    if (direction == 0) {
    ...
    void draw(float subframe) {
        ...
        if (p.attack_state() == 0) {
            ...
            spr.draw_world(layer, player_sublayer, p.sprite_index(), state_timer, 1, px+offset_x, py+offset_y, 0, p.face(), 1, 0xFFFFFFFF);
        } else {
            spr.draw_world(layer, player_sublayer, p.attack_sprite_index(), p.attack_timer(), 1, px+offset_x, py+offset_y, 0, p.attack_face(), 1, 0xFFFFFFFF);
        }
        // Draw the effects
